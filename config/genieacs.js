...
const genieacsApi = {
    ...
    async findDeviceByPhoneNumber(phoneNumber) {
        try {
            ...
            if (!response.data || response.data.length === 0) {
                // Jangan throw error di sini agar flow pencarian bisa lanjut ke metode lain (jika ada)
                // Tapi untuk backward compatibility yang mengharapkan error, kita biarkan atau return null?
                // Existing code expects throw.
    ...
    async findDeviceByPPPoE(pppoeUsername) {
        try {
            console.log(`Mencari perangkat dengan PPPoE Username: ${pppoeUsername}`);

            // Query berbagai path yang mungkin menyimpan username PPPoE
            // GenieACS MongoQuery syntax support $or
            const query = {
                $or: [
                    { "VirtualParameters.pppoeUsername._value": pppoeUsername },
                    { "VirtualParameters.pppoeUsername": pppoeUsername }, // Sometimes without _value
                    { "InternetGatewayDevice.WANDevice.1.WANConnectionDevice.1.WANPPPConnection.1.Username._value": pppoeUsername },
                    { "Device.PPP.Interface.1.Username._value": pppoeUsername }
                ]
            };
            ...
            if (!response.data || response.data.length === 0) {
                console.log(`Tidak ada perangkat ditemukan untuk PPPoE: ${pppoeUsername}`);
                return null;
            }

            console.log(`Ditemukan ${response.data.length} perangkat untuk PPPoE: ${pppoeUsername}`);
            return response.data[0];
        } catch (error) {
            console.error(`Error finding device by PPPoE ${pppoeUsername}:`, error.response?.data || error.message);
            return null;
        }
    },
